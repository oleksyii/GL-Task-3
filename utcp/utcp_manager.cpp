...
std::vector<char*> splitDataIntoPackets(const char* data, size_t dataSize, size_t maxPacketSize) {
...
int UTCP::Send(char* data)
{
    ...
    std::vector<char*> packets = splitDataIntoPackets(data, dataSize, maxPacketSize);

    // Display the packets
    for (size_t i = 0; i < packets.size(); ++i) {
        std::cout << "Packet " << i + 1 << ": " << packets[i] << std::endl;
    }
